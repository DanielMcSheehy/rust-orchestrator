...
def _extract_code(fn: Callable) -> str:
    ...
    # Strip decorator lines (they reference loom_sdk, absent in the worker).
    start = 0
...
def _dep_id(dep: Union[str, "Task", Callable]) -> str:
    ...
    if isinstance(dep, Task):
        return dep.id
    loom_task = getattr(dep, "loom_task", None)
    if isinstance(loom_task, Task):
        return loom_task.id
...
def task(
    ...
    *,
    ...
    working as a normal Python callable and gains a ``.loom_task``
...
class Flow:
    ...
    def add(self, t: Union[Task, Callable], **overrides: Any) -> "Flow":
        if not isinstance(t, Task):
            loom_task = getattr(t, "loom_task", None)
            if not isinstance(loom_task, Task):
