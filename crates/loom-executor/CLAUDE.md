# loom-executor — worker protocol, pool, isolation
...
- `entry` is the path *as the worker sees it* — host path in process mode,
  `/loom/job/<file>` inside containers/microVMs. Never leak host paths into
  sandboxes.
...
- `loom.py` / `loom.mjs` are the in-task platform bindings (`import loom`
  / `globalThis.loom`): query/ingest/invoke against `LOOM_API_URL`.
...
- Workers retire after `LOOM_WORKER_MAX_JOBS` (default 128) — this bounds
  Node's ESM module cache, which grows because every job is a unique URL.
  At most `LOOM_WORKER_MAX_IDLE` (8) stay parked per runtime.
...
- Container/microVM: `run --rm -i --network none --read-only --tmpfs /tmp
  ...
  `/loom/shim` and `/loom/job`. microVM = same invocation + `--runtime
