...
use axum::Json;
use chrono::Utc;
use loom_core::{Notebook, Runtime, WorkflowSpec};
use loom_executor::ExecRequest;
...
async fn dispatch(state: &SharedState, tool: &str, args: Value) -> Result<Value, String> {
    ...
    match tool {
        ...
        "create_workflow" => {
            ...
            loom_core::validate_dag(&spec.tasks).map_err(|e| err(&e))?;
        ...
        "create_function" => {
            let spec: loom_core::FunctionSpec = serde_json::from_value(json!({
                "name": args["name"],
