//! loom-executor — runs Python and TypeScript/JavaScript workloads in
...
const PYTHON_BINDINGS: &str = include_str!("../shims/loom.py");
const NODE_BINDINGS: &str = include_str!("../shims/loom.mjs");
...
impl Executor {
    /// Isolation mode is read from the environment (`LOOM_ISOLATION` and
    ...
    pub fn with_isolation(isolation: Isolation) -> Result<Self, ExecError> {
        let shim_dir = tempfile::Builder::new().prefix("loom-shims-").tempdir()?;
        std::fs::write(shim_dir.path().join("worker.py"), PYTHON_SHIM)?;
        std::fs::write(shim_dir.path().join("worker.mjs"), NODE_SHIM)?;
        std::fs::write(shim_dir.path().join("loom.py"), PYTHON_BINDINGS)?;
        std::fs::write(shim_dir.path().join("loom.mjs"), NODE_BINDINGS)?;
        let pool_enabled = std::env::var("LOOM_WORKER_POOL")
            .map(|v| v != "0")
            .unwrap_or(true);
    ...
    async fn execute_oneshot(
        ...
        let nonce = job_dir
            ...
            .file_name()
            .and_then(|n| n.to_str())
            .and_then(|n| n.strip_prefix("loom-job-"))
            .unwrap_or("0")
            .to_string();
...
fn write_job_dir(req: &ExecRequest) -> Result<(tempfile::TempDir, &'static str), ExecError> {
    let job_dir = tempfile::Builder::new().prefix("loom-job-").tempdir()?;
