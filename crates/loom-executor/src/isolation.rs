...
//!
//! Three tiers, selected via `LOOM_ISOLATION`:
//!
...
impl Isolation {
    ...
    pub fn from_env() -> Result<Self, IsolationError> {
        ...
        let mode = get("LOOM_ISOLATION").unwrap_or_else(|| "process".into());
        let container_config = || {
            let mut cfg = ContainerConfig {
                runtime: get("LOOM_VM_RUNTIME"),
                ..ContainerConfig::default()
            };
            if let Some(v) = get("LOOM_CONTAINER_ENGINE") {
                cfg.engine = v;
            }
            if let Some(v) = get("LOOM_PYTHON_IMAGE") {
                cfg.python_image = v;
            }
            if let Some(v) = get("LOOM_NODE_IMAGE") {
                cfg.node_image = v;
            }
            if let Some(v) = get("LOOM_WORKER_MEMORY") {
                cfg.memory = v;
            }
            if let Some(v) = get("LOOM_WORKER_CPUS") {
                cfg.cpus = v;
            }
            cfg
        };
        match mode.as_str() {
            "process" => Ok(Isolation::Process {
                python_bin: get("LOOM_PYTHON_BIN").unwrap_or_else(|| "python3".into()),
                node_bin: get("LOOM_NODE_BIN").unwrap_or_else(|| "node".into()),
            }),
...
pub fn plan(
    ...
    match isolation {
        ...
        Isolation::Container(cfg) | Isolation::MicroVm(cfg) => {
            let name = format!("loom-worker-{nonce}");
...
mod tests {
    ...
    fn container_mode_sandboxes_and_mounts() {
        ...
        assert!(joined.starts_with("run --rm -i --name loom-worker-abc123"));
    ...
    fn microvm_mode_selects_vm_runtime_class() {
        ...
        assert!(joined.contains(
            "node:22-slim node --experimental-strip-types --no-warnings /loom/shim/worker.mjs"
        ));
        assert_eq!(p.entry, "/loom/job/job.ts");
    }

    #[test]
    fn from_env_defaults_to_process() {
        // Only meaningful when the var is unset in the test environment.
        if std::env::var("LOOM_ISOLATION").is_err() {
